    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function releaseGrabbed() {
                if (grabbed) {
                    grabbed.vy = grabbed.savedVy != null ? grabbed.savedVy : (Math.random() * 0.4) + 0.1;
                    grabbed = null;
                }
            ...
            window.addEventListener('mouseup', releaseGrabbed);
            window.addEventListener('mouseleave', releaseGrabbed);
            ...
            function draw() {
                ...
                nodes.forEach(n => {
                    ...
                    if (dist < GRAB_RADIUS) nearGrabbable = true;
                    ...
                    if (n === grabbed) {
                        ...
                        ctx.fillStyle = '#60A5FA'; // grabbed accent
                ...
                if (!grabbed) document.body.style.cursor = nearGrabbable ? 'grab' : '';
...
const PARTICLE_DRIFT_DEFINITION: EffectDefinition = {
  ...
  supportsMode: true,
  background: (mode) => (mode === "light" ? LIGHT_PAPER : "#3f3f46"),
  ...
  patch(source, { size, length, density, mode }) {
    ...
    if (mode === "light") {
      next = next
        ...
        .replace(
          "ctx.fillStyle = '#60A5FA'; // grabbed accent",
          "ctx.fillStyle = '#2563EB'; // grabbed accent",
        );
    }
    return next;
  },
